...
export function FullPipelineView({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-0 overflow-hidden">
      ...
      >
      {/* Header card */}
      <div className="rounded-xl px-3 py-2.5" style={{ borderLeft: '3px solid var(--oc-accent)', backgroundColor: 'var(--oc-accent-soft)' }}>
        <div className="flex items-center gap-2">
          <div className="flex-1">
            <h2 className="text-base font-bold text-(--oc-accent-ink)">Full Pipeline</h2>
            <p className="text-xs text-(--oc-accent-ink)" style={{ opacity: 0.7 }}>
              Cross-stage control center{activeCampaignName ? ` · ${activeCampaignName}` : ''}{companies?.total != null ? ` · ${companies.total.toLocaleString()} domains` : ''}
            </p>
          </div>
          <div className="relative">
            ...
            <input
              ...
              className="rounded-lg border border-(--oc-border) bg-(--oc-surface) py-1.5 pl-7 pr-3 text-xs outline-none transition focus:border-(--oc-accent) focus:bg-white disabled:cursor-not-allowed disabled:opacity-60"
              style={{ width: 180 }}
            />
          </div>
          <button
            ...
            className="rounded-lg border border-(--oc-accent) bg-(--oc-accent-soft) px-3 py-1.5 text-xs font-semibold text-(--oc-accent-ink) transition hover:bg-(--oc-accent-soft)/80 disabled:opacity-60 whitespace-nowrap"
          >
