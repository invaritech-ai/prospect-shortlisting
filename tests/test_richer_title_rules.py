...
def test_regex_rule_matches(db_session: Session) -> None:
    ...
    db_session.commit()
    result = run_title_test(TitleTestRequest(title="Head of Digital Commerce"), session=db_session)
    assert result.matched is True


def test_seniority_c_level_matches_ceo(db_session: Session) -> None:
    db_session.add(TitleMatchRule(rule_type="include", keywords="c_level", match_type="seniority"))
    db_session.commit()
    result = run_title_test(TitleTestRequest(title="Chief Executive Officer"), session=db_session)
    assert result.matched is True


def test_seniority_vp_level_matches_vice_president(db_session: Session) -> None:
    db_session.add(TitleMatchRule(rule_type="include", keywords="vp_level", match_type="seniority"))
    db_session.commit()
    result = run_title_test(TitleTestRequest(title="Vice President of Sales"), session=db_session)
    assert result.matched is True


def test_keyword_still_works_without_match_type(db_session: Session) -> None:
    """Default match_type='keyword' behaviour is unchanged."""
    db_session.add(TitleMatchRule(rule_type="include", keywords="marketing, director"))
    db_session.commit()
