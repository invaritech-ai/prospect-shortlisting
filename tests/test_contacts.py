...
def test_list_contacts_supports_letters_filter(db_session: Session) -> None:
    ...
    try:
        ...
        db_session.exec(delete(Contact).where(col(Contact.company_id).in_(
            select(Company.id).where(col(Company.upload_id) == upload.id)
        )))
        db_session.exec(delete(ContactFetchJob).where(col(ContactFetchJob.company_id).in_(
            select(Company.id).where(col(Company.upload_id) == upload.id)
        )))
...
def test_stats_validation_scope_honors_upload(db_session: Session) -> None:
    ...
    try:
        ...
        db_session.exec(delete(ContactVerifyJob))
        db_session.exec(delete(Contact).where(col(Contact.company_id).in_(
